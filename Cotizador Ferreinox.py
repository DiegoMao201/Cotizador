...
def generar_pdf_profesional(cliente, items_df, subtotal, descuento_total, iva_valor, total_general, observaciones):
    ...
    pdf.set_y(200); pdf.set_font('Helvetica', 'B', 10); pdf.cell(0, 7, 'Observaciones Adicionales:', 0, 1)
...
def cargar_y_procesar_datos_completos():
    ...
    sin_stock = df_final[df_final[INVENTARIO_COL] <= 0].shape[0] if INVENTARIO_FILE_PATH.exists() else 0
...
if df_productos is None: st.error("No se pudo cargar el archivo de precios. La aplicación no puede continuar."); st.stop()
...
with st.sidebar:
    ...
    with st.expander("Diagnóstico de Archivos", expanded=False):
        st.write(f"Logo (`{LOGO_FILE_NAME}`): { '✅ Encontrado' if LOGO_FILE_PATH.exists() else '❌ NO ENCONTRADO'}")
        st.write(f"Pie de Página (`{FOOTER_IMAGE_NAME}`): { '✅ Encontrado' if FOOTER_IMAGE_PATH.exists() else '❌ NO ENCONTRADO'}")
        st.write(f"Clientes (`{CLIENTES_FILE_NAME}`): { '✅ Encontrado' if CLIENTES_FILE_PATH.exists() else '❌ NO ENCONTRADO'}")
        st.write(f"Precios (`{PRODUCTOS_FILE_NAME}`): { '✅ Encontrado' if PRODUCTOS_FILE_PATH.exists() else '❌ NO ENCONTRADO'}")
        st.write(f"Inventario (`{INVENTARIO_FILE_NAME}`): { '✅ Encontrado' if INVENTARIO_FILE_PATH.exists() else '⚠️ No se usará inventario'}")
        if INVENTARIO_FILE_PATH.exists(): st.info(f"Con Stock: {con_stock} | Sin Stock: {sin_stock}")
...
with st.container(border=True):
    ...
    with tab_existente:
        if df_clientes is not None:
            ...
            cliente_sel_nombre = st.selectbox("Clientes guardados:", lista_clientes, index=0, help="Seleccione un cliente de la lista para cargar sus datos.")
        ...
        else: st.info("No se pudo cargar el archivo de clientes. Puede registrar uno nuevo en la siguiente pestaña.")
    with tab_nuevo:
    ...
    if not st.session_state.cotizacion_items: st.info("El carrito está vacío.")
        ...
        with col2:
            if st.session_state.cliente_actual:
                ...
                st.warning("Seleccione un cliente para poder descargar.")
