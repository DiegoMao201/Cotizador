...
# --- FUNCIONES DE CARGA Y VERIFICACIÓN (CORREGIDAS) ---
@st.cache_data
def cargar_datos(path_archivo, columnas_num):
    nombre_archivo = path_archivo.name
    if not path_archivo.exists():
        st.error(f"Error Crítico: No se encontró el archivo '{nombre_archivo}'.")
    ...
    try:
        ...
        st.error(f"Ocurrió un error detallado al intentar leer '{nombre_archivo}':")
        st.exception(e)
        return None
...
# LA LIMPIEZA DE DATOS AHORA SE APLICA SÓLO AL DATAFRAME DE PRODUCTOS
...
with st.container(border=True):
    ...
    with tab_nuevo:
        with st.form("form_nuevo_cliente"):
            nombre_nuevo = st.text_input(f"{CLIENTE_NOMBRE_COL}*"); nit_nuevo = st.text_input(CLIENTE_NIT_COL)
            tel_nuevo = st.text_input(CLIENTE_TEL_COL); dir_nueva = st.text_input(CLIENTE_DIR_COL)
    ...
    if producto_sel_str:
        ...
        with col_cant_add:
            ...
            if st.button("➕ Agregar al Carrito", use_container_width=True, type="primary"):
                ...
                }); st.toast(f"✅ '{info_producto[NOMBRE_PRODUCTO_COL]}' agregado!", icon="🛒"); st.rerun()
with st.container(border=True):
    ...
    if not st.session_state.cotizacion_items:
        ...
        with col_desc:
            st.subheader("Descuento"); descuento_opciones = {f"{i}%": i/100.0 for i in range(21)}
        ...
        with col_totales:
            ...
            m1, m2 = st.columns(2)
            m1.metric("Subtotal", f"${subtotal:,.2f}"); m1.metric("Descuento ({:.0%})".format(st.session_state.descuento_pct), f"-${descuento_valor:,.2f}")
            m2.metric("IVA (19%)", f"${iva_valor:,.2f}"); m2.metric("Total General", f"${total_general:,.2f}")
        st.divider()
        ...
        with col_descargar:
            ...
            else: st.warning("Seleccione un cliente para poder descargar la cotización.")
