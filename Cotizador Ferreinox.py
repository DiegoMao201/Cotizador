...
def cargar_y_procesar_datos():
    ...
    if not INVENTARIO_FILE_PATH.exists():
        df_prods[INVENTARIO_COL] = -1 # Marcar con -1 si no hay archivo de inventario
        return df_prods
...
df_productos_bruto = cargar_y_procesar_datos()
df_clientes = pd.read_excel(CLIENTES_FILE_PATH) if CLIENTES_FILE_PATH.exists() else None
...
with st.container(border=True): # Agregar Productos
    ...
    if producto_sel_str:
        ...
        # Alerta de inventario en la app
        if info_producto[INVENTARIO_COL] <= 0:
        ...
        with col2:
            ...
            if st.button("➕ Agregar", use_container_width=True, type="primary"):
                ...
                st.session_state.cotizacion_items.append({
                    ...
                    "Inventario": info_producto[INVENTARIO_COL] # Guardar el estado del inventario
    ...
    if not st.session_state.cotizacion_items: st.info("El carrito está vacío.")
        ...
        edited_df = st.data_editor(pd.DataFrame(st.session_state.cotizacion_items),
            column_config={
                ...
                "Inventario": None # Ocultar columna de inventario en la tabla
            },
